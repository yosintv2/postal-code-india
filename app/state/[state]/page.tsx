...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title, description: desc, alternates: { canonical: `/state/${slug}/` }, openGraph: { title, description: desc } };
}
...
export default async function StatePage({ params }: Props) {
  ...
  const faqItems = [
    { q: `What are the PIN codes in ${stateData.stateName}?`, a: `${stateData.stateName} has ${stateData.totalOffices.toLocaleString()} post offices across ${stateData.totalDistricts} districts. Browse the districts above to find PIN codes for each area.` },
    ...
    { q: `How do I find a PIN code in ${stateData.stateName}?`, a: `Click on any district above to see all PIN codes and post offices in that district of ${stateData.stateName}.` },
