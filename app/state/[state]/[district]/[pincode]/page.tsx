...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title,
    description: desc,
    keywords: `${pincode} pin code, pin code ${pincode}, ${district.districtName} pin code, ${pincode} area pin code india, what is pin code ${pincode}, pin code of ${district.districtName} ${state.stateName}`,
...
export default async function PincodePage({ params }: Props) {
  ...
  const faqs = [
    { q: `What is PIN code ${pincode}?`, a: `PIN code ${pincode} is assigned to ${group.offices.length} post office${group.offices.length > 1 ? 's' : ''} in ${district.districtName}, ${state.stateName}, India. It is a 6-digit India Post code used to route mail to this area.` },
    ...
    { q: `Is PIN code ${pincode} a delivery PIN?`, a: group.offices.some(o => o.deliveryStatus === 'Delivery') ? `Yes, PIN code ${pincode} has post offices with active mail delivery status.` : `PIN code ${pincode} is primarily a non-delivery administrative code.` },
    ...
    { q: `What does H.O mean for PIN code ${pincode}?`, a: `H.O stands for Head Office — the main post office in a district. ${headOffice ? `${headOffice.officeName} is the Head Office at PIN code ${pincode}.` : `PIN code ${pincode} does not have a Head Office.`}` },
    ...
    <>
      ...
      <section className="postal-detail">
        <div className="postal-code-hero">
          ...
          {mainOffice && (
            <span className={`type-badge-lg type-${getTypeKey(mainOffice.officeType)}`}>
              ...
              <tbody>
                {group.offices.map((o, i) => (
                  <tr key={i} className={o.officeType === 'H.O' ? 'row-ho' : ''}>
                    ...
                    <td><span className={`type-badge type-${getTypeKey(o.officeType)}`}>{o.officeType}</span></td>
                    <td>
            ...
            <div className="nearby-list">
              {nearbyPincodes.map(p => (
                <Link key={p.pincode} href={`/state/${stateSlug}/${districtSlug}/${p.pincode}/`} className="nearby-item">
                  ...
                  {p.hasHeadOffice && <span className="nearby-meta">H.O</span>}
                </Link>
              ))}
            </div>
